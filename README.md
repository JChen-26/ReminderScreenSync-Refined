...
- **菜单栏常驻时隐藏 Dock 图标**：关闭主窗口后 Dock 自动隐藏，打开主窗口后恢复显示
- **菜单栏设置面板**：新增开机自动启动、启动时自动同步、隐藏 Dock 三个快捷开关
- 修复 `autoStartSync` 和 `LaunchService` 此前有代码但无 UI 的问题
- 优化窗口关闭检测与 Dock 策略切换逻辑

### v1.0

- 初始版本：多设备多列表同步、菜单栏常驻、开机自启动、同步日志、冲突处理
